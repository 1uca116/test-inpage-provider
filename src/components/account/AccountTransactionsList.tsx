...
const TransactionInfo = (props: ITransactionProps) => {
    ...
    return (
        <tr>
            <td className='p-2 border-2 '>{unixDateToStr(props.item.createdAt, 'DD.MM.YY HH:mm:ss')}</td>
            <td className='p-2 border-2 '>{props.item.inMessage.value !== '0' ? (<>{Number(props.item.inMessage.value) / 1000000000} EVER</>)
                : ''}</td>
            <td className='p-2 border-2 '>{props.item.outMessages?.map(x => x.value ?
                (<>{props.item.outMessages.map((x) => Number(x.value) / 1000000000)} EVER</>) : ('')
            )
            }</td>
        </tr>
    )
