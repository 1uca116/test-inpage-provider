  ...
  useEffect(() => {
    async function getTokenBalance() {
      ...
      if (auth.account) {
        ...
        const tokenDecimalsString = await contract.methods
          .decimals({ answerId: 0 })
          .call({ responsible: true });

        const tokenDecimals = Number.parseInt(tokenDecimalsString.value0);

        const tokenSymbol = await contract.methods
          .symbol({ answerId: 0 })
          .call({ responsible: true });

        let resultBalance = '0';
        try {
          const tokenWalletContract = new ever.Contract(
            tokenWalletAbi,
            tokenWalletAddress.value0
          );
          const tokenBalance = await tokenWalletContract.methods
            .balance({
              answerId: 0,
            })
            .call({
        ...
        const balance = new BigNumber(resultBalance).div(
